...
Asset Tracker Pro is a professional employee and equipment management application designed for construction and demolition industries. It provides drag-and-drop functionality for assigning employees and equipment to projects, real-time conflict detection, and comprehensive asset management. Built as a full-stack web application with a React frontend and Express backend, it features a modern dark-themed UI optimized for operational workflows. Key capabilities include enterprise-grade conflict management with real-time detection and professional alerts, a three-panel layout for streamlined project assignments, JWT authentication, an enhanced navigation header, an advanced white-label system with comprehensive page-level branding support, object storage integration for assets, a comprehensive settings system, a professional directory system with profile management, a guided profile builder wizard, complete archive/history tracking with audit trails, and live assignment synchronization with enhanced SSE real-time capabilities including event replay, toast notifications, and smooth drag-and-drop animations. The system includes a comprehensive Stripe billing integration with three pricing tiers (Starter $199/mo, Growth $399/mo, Enterprise $799/mo), plan-based feature gates, and complete subscription management. The application ensures live data creation and an integrated workflow between settings management and the operational dashboard, with authentic equipment data imported from Excel spreadsheets for realistic testing scenarios. The system now supports enterprise-grade real-time collaboration with event ID tracking, missed event replay, and professional user feedback systems. **Recent Fix (Aug 12, 2025): Successfully resolved drag-and-drop cache synchronization issues with unified query key architecture. Assets now immediately disappear from dashboard lists when assigned and appear correctly on project detail pages with consistent real-time updates.**
